...
def main() -> None:
    ...
    logger.debug(f"Reddit Username: u/{config.reddit_username}")
    logger.debug(f"Debug Mode: {config.debug_mode}")
    logger.debug(f"Full Tracebacks: {config.full_tracebacks}")
    logger.debug(f"Parse Body: {config.parse_body}")
    logger.debug(f"Parse Imgur Links: {config.parse_imgur_links}")
    logger.debug(f"Send Test Webhooks: {config.send_test_webhooks}")
    logger.debug(f"Combine Images: {config.combine_images}")
    logger.debug(f"Ping for Warnings: {config.ping_for_warnings}")
    logger.debug(f"Check If Post Was Deleted: {config.check_if_post_was_deleted}")
    logger.debug(f"File Logging: {config.file_logging}")
    logger.debug(f"Check USL: {config.check_usl}")
    logger.debug(f"Configured {len(config.pings)} ping categories")
    logger.debug(f"Configured global blocklist with {len(config.global_blocklist)} patterns")
